...
class Train(object):
    ...
    def __init__(self,
        ...
        self.model = HRNet(c=self.model_c, nof_joints=self.model_nof_joints,
                           bn_momentum=self.model_bn_momentum,retain_block = EP_list).to(self.device)
    ...
    def _checkpoint(self):
        ...
        save_checkpoint(path=os.path.join(self.log_path, 'checkpoint_last.pth'), epoch=self.epoch + 1, model=self.model,
                        optimizer=self.optim, params=self.parameters,best_Acc= self.best_acc,best_mAP =self.best_mAP,EP = self.EP_list)
