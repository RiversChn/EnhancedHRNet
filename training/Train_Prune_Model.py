...
class Prune_Train(Train):
    ...
    def __init__(self,
        ...
        Args:
            ...
            device (torch.device): device to be used (default: cuda, if available).
                Default: None
        """
        super(Prune_Train, self).__init__(
            exp_name=exp_name,
            ds_train=ds_train,
            ds_val=ds_val,
            epochs=epochs,
        ...
        self.techer_model_path = techer_model_path
    ...
    def _train(self):
        ...
        teacher_model = HRNet(48, 17, 0.1,retain_block)
        print(self.techer_model_path)
        checkpoint = torch.load( self.techer_model_path)

        old_state_dict = checkpoint['model']
        teacher_model.load_state_dict(old_state_dict)
        ...
        for step, (image, target, target_weight, joints_data) in enumerate(tqdm(self.dl_train, desc='Training')):
            ...
            soft_targets = teacher_model(image)

            loss1 = self.loss_fn(output, target, target_weight)
            loss2 = self.loss_fn(output, target, target_weight)
